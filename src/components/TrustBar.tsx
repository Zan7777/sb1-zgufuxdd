  ...
  return (
    <div className="border-y border-gray-200 bg-gradient-to-r from-gray-50 via-white to-gray-50 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-2 sm:py-6">
        <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-6 lg:gap-8">
          {trustItems.map((item, index) => {
            const Icon = item.icon;
            return (
              <div key={index} className="flex items-center space-x-1 sm:space-x-2 text-gray-600 hover:text-gray-900 transition-colors duration-300">
                <Icon className="w-3 h-3 sm:w-5 sm:h-5 text-blue-600" />
                <span className="font-medium whitespace-nowrap" style={{ fontSize: 'clamp(8px, 2.2vw, 14px)' }}>{item.label}</span>
              </div>
            );
          })}
        </div>
